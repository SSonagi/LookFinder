...
const Outfit = ({outfit}: {outfit: IOutfit}) => {
    return (
        ...
        >
            ...
            ] as [string, IPiece[], boolean][]).map(([position, data, double]) => (
                <div className='w-full'>
                    <div className='text-sm px-1 bg-primary w-min rounded-md border-2 text-[#F7F7F7]'>{position}</div>
                    <div className={'w-full flex flex-row flex-wrap justify-center items-center gap-3 ' + (double ? 'h-32' : 'h-16')}>
                        {data.slice(0, (double ? 6 : 3)).map((piece) => (
                            <Piece piece={piece}/>
                        ))}
                        {data.length > (double ? 6 : 3) &&
                            <div>
                                ...
                            </div>
                        }
